    ...
    def from_config(cls, cfg: dict[str, Any], session_dir: Path) -> "CalibrationSettings":
        ...
        if c_type == "convert":
            ...
            if method == "board":
                ...
                kwargs.update(
                    board_position         = board.get("board_position"),
...
class PointCalibration(Calibration):
    ...
    def calibrate_extrinsics(self, source):
        ...
        if self.settings.extrinsics_method == 'board':
            board_position = self.settings.board_position
            ...
            if board_position == 'horizontal':
                ...
                object_coords_3d[:, 1:] = np.mgrid[0:self.extrinsics_corners_nb[0], 0:self.extrinsics_corners_nb[1]][::-1].T.reshape(-1, 2)
                object_coords_3d[:, 1:] = object_coords_3d[:, 1:] * self.extrinsics_square_size
            else:
        ...
        if self.settings.show_reprojection_error:
            ...
            # save image
            reproj_img_path = os.path.join(os.path.dirname(source.extrinsic_files[0]), f'calib_{os.path.splitext(os.path.basename(source.extrinsic_files[0]))[0]}.png')
