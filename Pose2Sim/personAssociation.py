...
def triangulate_reproject_point(proposals, all_json_data_f, tracked_keypoint_id, P_all, calib_params, config):
    '''
    Triangulate and reproject a tracked point for all possible combinations of people.

    INPUTS:
    - proposals: array: list of lists of ints: combinations of persons' ids for each camera for each proposal
    - all_json_data_f: list of lists (n_cams*nb_persons): coordinates for each person in each camera
    ...
    OUTPUTS:
    - error_proposals: list of floats: reprojection error for each proposal
    - new_proposals: array: list of lists of ints: combinations of persons' ids for each camera for each proposal
    - Q_proposals: list of 4D arrays: 3D coordinates of the triangulated point for each proposal
    '''
    
    error_proposals, new_proposals, Q_proposals = [], [], []
    for proposal in proposals:
        # retrieve coordinates of tracked point
        coords_point = []
        for index_cam, person_nb in enumerate(proposal):
            try:
                coords_point.append(all_json_data_f[index_cam][int(person_nb)][tracked_keypoint_id*3:tracked_keypoint_id*3+3])
            except:
                coords_point.append([np.nan, np.nan, np.nan])
        coords_point = np.array(coords_point)
        
        # triangulate the tracked point for the selected combination
        error, proposal, Q = triangulate_comb(proposal, coords_point, P_all, calib_params, config)
        
        error_proposals.append(error)
        new_proposals.append(proposal)
        Q_proposals.append(Q)
    
    return error_proposals, new_proposals, Q_proposals
...
def track_2d_all(config):
    ...
    for f in tqdm(range(*f_range)):
        ...
        if not multi_person:
            ...
            # # triangulate and reproject Neck
            # error_proposals, new_proposals, Q_proposals = triangulate_reproject_point(proposals, all_json_data_f, tracked_keypoint_id, P_all, calib_params, config)
            
            # # reID persons across frames by checking the distance from one frame to another
            # Q_kpt, personsIDs_sorted, associated_tuples = sort_people(Q_kpt_old, Q_proposals)
            
            # error_proposals = np.array(error_proposals)[personsIDs_sorted]
            # new_proposals = np.array(new_proposals)[personsIDs_sorted]
            
            # # rearrange proposals, errors, 
            # new_new_proposals = []
            # for i in range(len(Q_kpt)):
                # id_in_old =  associated_tuples[:,1][associated_tuples[:,0] == i].tolist()
                # if len(id_in_old) > 0:
                    # new_new_proposals += [new_proposals[id_in_old[0]]]
                # else:
                    # new_new_proposals += [[np.nan]*n_cams]
                    # # new_new_proposals += [new_proposals[i]]
            # print(new_new_proposals)
            

            # print('personsIDs_sorted ', personsIDs_sorted)
            # error_proposals = np.array(error_proposals)[personsIDs_sorted]

            # proposals = np.array(new_new_proposals)[personsIDs_sorted]
